...
> 用户把这个仓库丢给你，意思是：**请你把 Scribe 装好并接上，然后开始用它记我们的工作进展。**
...
```bash
claude mcp add scribe -- scribe-mcp        # claude code
```

其它宿主手动加：

```json
{ "mcpServers": { "scribe": { "command": "scribe-mcp", "args": [], "env": {} } } }
```

codex 写进 `~/.codex/config.toml` 的 `[mcp_servers.scribe]`。

**验证**：重启会话后你应该能看到 `log_progress`、`catch_up`、`scribe_doctor` 等 8 个工具。

> 不需要另外起任何服务。Scribe 是单进程的，数据落在 `~/.scribe/`。

---

## Step 3 · 自检

调用 `scribe_doctor`。它会逐项告诉你什么配好了、什么还缺、缺的怎么补。
**后面几步就是把它标 ✗ 的项补齐。**
...
授权完成后 Scribe 会自动把用户名下在办的 issue 拉下来。

- 用不了浏览器（比如远程机器）→ 让用户跑 `scribe login --no-browser --assignee <邮箱>`，
  把打印出来的链接在自己电脑上打开。
- 用户更想直接给 token → `scribe login --token <token> --assignee <邮箱>`。

**不要替用户点授权，也不要把 token 写进任何文件或打印出来。**
凭证由 Scribe 自己存在 `~/.scribe/config.json`（权限 0600）。

**验证**：再调 `scribe_doctor`，Mobius 和 issue 缓存两项应该都变 ✓。
...
```bash
scribe setup --llm-url <base_url> --llm-key <key> --llm-model <model>
```
...
**验证**：`scribe_doctor` 的 LLM 一项变 ✓。

---

## Step 6 · 试一条

调用 `log_progress(content="装好了 scribe 并接上了 Mobius")`，看返回里的「配对方式」。
...
这部分比安装重要。Scribe 的价值全在于**你是否真的在记**。
